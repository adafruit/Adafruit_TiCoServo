...
/**
 * @brief Enable servo use on requested pin.  Must call this before any write()
operation.  Pin selection is VERY limited, see examples (or
known_16_bit_timers.h) for a list.
 * @param p The pin to use
 *
 */
...
/**
 * @brief Enable or disable PWM on pin.  Private method.  attach() already set
up control registers for no output, so switching on or off just needs to toggle
 ...
 *
 * @param pos The new servo position
 */
...
/**
 * @brief Stop servo pulse output on pin
 *
 */
...
/**
 * @brief Return servo position in degrees (**not available on ATtiny**)
 *
 * @return uint8_t The servo position in degrees
 */
...
/**
 * @brief Return servo position in microseconds (**not available on ATtiny**)
 *
 * @return uint16_t
 */
