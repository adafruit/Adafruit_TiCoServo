...
#endif
/**
 * @brief Class for controlling servos while using the Adafruit NeoPixel library
 *
 */
...
public:
  ...
  void attach(const int8_t pin), write(const servoPos_t pos);
  /**
   * @brief Return servo position
   ...
   * @return bool true: a pin is attached false: no pin is attached
   */
